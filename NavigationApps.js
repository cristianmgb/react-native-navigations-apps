...
class NavigationApps extends Component {
          ...
          ios: {
            maps: {
              ...
              navigateByLatAndLon: ({
                ...
                encodeURI(
                  this.state.navApps.maps.appDeepLinkUriToUse +
                    `daddr=${addressToNavigate}&dirflg=${travelMode}&ll=${llatan},${lon}`
                ),
              searchLocationByLatAndLon: ({ lat, lon }) =>
                encodeURI(
                  this.state.navApps.maps.appDeepLinkUriToUse +
  ...
  handleNavApp = async (navApp) => {
    ...
    try {
      ...
      if (!supported) {
        ...
        console.log(e),
        Alert.alert('Oops...', `${navApp} is not installed`);
    }
  };

  renderNavigationApps = () => {
    const { iconSize, externalNav } = this.props;
    const { navApps } = this.state;
    return Object.keys(navApps).map((navApp, key) => {
      ...
      return (
        <TouchableOpacity
          onPress={() => {
            externalNav(navAppItem.title);
            this.handleNavApp(navApp);
          }}
          key={key}
        >
          <Image
  ...
  renderNavigationAppsAsActionSheet = () => {
    ...
    const actionSheetOptions = () => {
      const { actionSheetBtnCloseTitle, externalNav } = this.props;
      const { navApps } = this.state;
        ...
        <ActionSheet
          ...
          onPress={async (index) => {
            if (index !== actionSheetOptions().length - 1) {
              ...
              externalNav(actionSheetOptions()[index]);
            }
          }}
        />
      </React.Fragment>
